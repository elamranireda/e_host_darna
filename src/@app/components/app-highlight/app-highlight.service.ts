...
export class AppHighlightService {
  constructor(@Inject(APP_HIGHLIGHT_OPTIONS) options: AppHighlightOptions) {
    if (options) {
      // Register HighlightJS languages
      options
        .languages?.()
        .map((language: AppHighlightLanguage) =>
   ...
   */
  highlightAuto(value: string, languageSubset: string[]): AppHighlightResult {
   ...
   */
  configure(config: AppHighlightConfig) {
